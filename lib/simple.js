 ...
 * Modificato da:
 * - https://github.com/troncarlo
 * - https://github.co/troncarlo/zyk-bot
*/
...
export function smsg(conn, m, store) {
  ...
  if (m.message) {
    ...
    if (m.message.mentionedJid) {
      m.mentionedJid = m.message.mentionedJid.map(jid => {
        if (!jid) return jid
        if (!jid.endsWith('@lid')) return normalizeJid(jid, conn)
        if (conn?.chats) {
          for (const [chatJid, chat] of Object.entries(conn.chats)) {
            if (!chatJid.endsWith('@s.whatsapp.net')) continue
            const chatLid = chat.lid
            if (chatLid && chatLid.split('@')[0] === jid.split('@')[0]) return chatJid
          }
        }
        return jid
      })
    }
    if (m.message.mentionedJid && m.mentionedJid?.length) {
      const rawJids = m.message.mentionedJid
      let txt = m._text || m.text || ''
      rawJids.forEach((rawJid, i) => {
        const resolved = m.mentionedJid[i]
        if (!resolved || rawJid === resolved) return
        const rawNum = rawJid.split('@')[0]
        const resolvedNum = resolved.split('@')[0]
        txt = txt.replaceAll(`@${rawNum}`, `@${resolvedNum}`)
      })
      m._text = txt
    }
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    quoted: {
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {text: q} : q)), {
         ...
         mentionedJid: {
    get() {
        const raw = this.msg?.contextInfo?.mentionedJid?.length 
        ...
        return raw.map(jid => {
            ...
            if (this.conn?.chats) {
                for (const [chatJid, chat] of Object.entries(this.conn.chats)) {
                    if (!chatJid.endsWith('@s.whatsapp.net')) continue
                    const chatLid = chat.lid
                    if (chatLid && chatLid.split('@')[0] === jid.split('@')[0]) return chatJid
                }
    ...
    get() {
        ...
        return raw.map(jid => {
            ...
            if (self.conn?.chats) {
                for (const [chatJid, chat] of Object.entries(self.conn.chats)) {
                    if (!chatJid.endsWith('@s.whatsapp.net')) continue
                    const chatLid = chat.lid
                    if (chatLid && chatLid.split('@')[0] === jid.split('@')[0]) return chatJid
                }
